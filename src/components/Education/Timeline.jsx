            ...
            <div className={`ml-12 md:ml-0 w-full md:w-1/2 ${
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg card-hover">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xl font-bold">{item?.degree || item?.position || 'Title'}</h3>
                  <span className="text-sm text-primary-500">
                    {item?.startDate ? moment(item.startDate).format('YYYY') : 'N/A'} - {item?.isCurrent ? 'Present' : (item?.endDate ? moment(item.endDate).format('YYYY') : 'N/A')}
                  </span>
                </div>
                <p className="text-gray-600 dark:text-gray-400 font-semibold mb-2">
                  {item?.institution || item?.company || 'Institution'}
                </p>
                {item?.field && (
